...
function Hamburger(buns, garnishes, cheeses, sauces, patty, pattyCount, extras) {
    this.buns = buns;
    ...
    this.describe = function() {
        let description = `This hamburger comes with a ${this.buns} bun, ` +
                          `${this.pattyCount} ${this.patty} patty(ies), `;
        ...
        if (this.extras.length) {
            description += `and extras (${this.extras.join(", ")})`;
        }
        return description.trim().replace(/,\s*$/, ""); 
    };
}

// Creating a hamburger object with various options
var myHamburger = new Hamburger(
    "sesame seed", // buns
    ["lettuce", "tomato", "onion"], // garnishes
    ["cheddar", "swiss"], // cheeses
    ["mayo", "ketchup"], // sauces
    "beef", // patty
    2, // patty count
    ["pickles", "hot peppers", "olives"] // extras
);
